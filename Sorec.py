...
class Sorec():
    ...
    def __init__(self, rating_tuples, social_tuples, latent_d=10, lambda_c = 0.1, lambda_u = 0.1, lambda_v = 0.1, lambda_z = 0.1, Wm=0.0002, Ws = 0.0002):
        ...
        self.social = numpy.array(social_tuples).astype(float)
        ...
        self.social = numpy.random.random((self.num_users, self.latent_d))
        ...
        self.new_social = numpy.random.random((self.num_users, self.latent_d))
    ...
    def loss(self, users=None, items=None, social = None):
        ...
        for rating_tuple in self.social:
            ...
            r_hat = numpy.sum(users[i] * social[j])
            if rating == 0.0 :
    ...
    def update(self):
        ...
        updates_z = numpy.zeros((self.num_users, self.latent_d))
        ...
        for rating_tuple in self.social:
            ...
            r_hat = numpy.sum(self.users[i] * self.social[j])
    ...
    def apply_updates(self, updates_u, updates_v, updates_z):
        ...
        for i in range(self.num_users):
            for d in range(self.latent_d):
                self.social[i, d] = self.new_social[i, d]
    ...
    def try_updates(self, updates_u, updates_v, update_z):
        ...
        for i in range(self.num_users):
            for d in range(self.latent_d):
                self.new_social[i,d] = self.social[i, d] + \
                                       alpha * (-self.lambda_u * self.social[i, d] + 2*update_z[i, d])
...
if __name__ == "__main__":
    ...
    pmf = Sorec(ratings, socials, latent_d=5, lambda_c = 0.1, lambda_u = 0.1, lambda_v = 0.1, lambda_z = 0.1, Wm = 0.0, Ws = 0.0)
